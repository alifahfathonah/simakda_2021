...
class Dynamic_menu {
    ...
    private $style        = 'style="padding: 10px"';
...
//	$otori = $this->session->userdata('pcOtoriName');
	$CI =& get_instance();
	$otori = $CI->session->userdata('pcOtoriName');//berdasarkan type
	$otori2 = $CI->session->userdata('pcUser');//berdasarkan id user
	//echo "otori ==  ($otori)<br />";
    ...
    function get_childs($menu, $parent_id, $tampung)
    {
		...
		$xid = $tampung[$a];
            ...
            if ($menu[$xid ]['show'] && $menu[$xid ]['parent'] == $parent_id)    // are we allowed to see this menu?
            {
                ...
                {
                    $html_out .= "\t\t\t\t\t\t".'<li>'.anchor($menu[$xid ]['url'], '<span style="padding:5px">'.$menu[$xid ]['title'].'</span>');
                }
