...
function preview_rincian_belanja_skpd_pergeseran(){
           ...
           $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat as pangkat FROM ms_ttd WHERE (REPLACE(id_ttd, ' ', 'a')='$ttd1' )  ";
            ...
            $sqltp="SELECT SUM(nilai$status_anggaran1) AS totb, SUM(nilai$status_anggaran2) AS totb2 FROM trdrka WHERE left(kd_sub_kegiatan,12)='$giat' AND left(kd_skpd,17)=left('$id',17)";
                     ...
                     foreach ($sqlbsub->result() as $rowsub)
                    {
                     ...
                     foreach($kodesumberdana->result() as $oke){
                        ...
                        $cRet .= "<table style='border-collapse:collapse;font-size:10px' width='100%' align='center' border='1' cellspacing='2' cellpadding='5'>
                            ...
                            $sql1="SELECT * FROM(SELECT 0 header,0 no_po, LEFT(a.kd_rek6,1)AS rek1,LEFT(a.kd_rek6,1)AS rek,b.nm_rek1 AS nama, '' spek, '' as koefisien,0 AS volume,' 'AS satuan, 0 AS harga, '' spek2, '' as koefisien2,0 AS volume2,' 'AS satuan2, 0 AS harga2,
                                    ...
                                    SUBSTRING(a.no_trdrka,24,15)='$sub' GROUP BY RIGHT(a.no_trdrka,12),b.header, b.no_po,b.uraian)z WHERE header='1' 
                                        ...
                                        UNION ALL
                                        SELECT a. header,a.no_po as no_po,RIGHT(a.no_trdrka,12) AS rek1,' 'AS rek,a.uraian AS nama,spesifikasi$status_speksifikasi1 as spek, CAST(a.volume$status_volume1 as varchar)+' '+a.satuan$status_satuan1 koefisien1, a.volume$status_volume1 AS volume,a.satuan$status_satuan1 AS satuan, a.harga$status_harga1 AS harga, a.spesifikasi$status_speksifikasi2 as spek, CAST(a.volume$status_volume2 as varchar)+' '+a.satuan$status_satuan2 koefisien2, a.volume$status_volume2 AS volume,a.satuan$status_satuan2 AS satuan, harga$status_harga2 AS harga,
        ...
        if($ttd1!='tanpa'){
            ...
            $tambahan="";
                ...
                $angkas5=$this->db->query("SELECT  kd_skpd, 
                                                ...
                                                select bulan, left(kd_skpd,17)+'.0000' kd_skpd , sum(nilai$status_anggaran2) nilai from trdskpd_ro WHERE left(kd_rek6,1)='5' and left(kd_subkegiatan,len('$giat'))='$giat' GROUP BY bulan, left(kd_skpd,17)
